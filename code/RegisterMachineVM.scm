...
(define (make-stack)
  (let ((s '()))
    ...
    (define (pop)
      (if (null? s)
        (error "Operation on Empty Stack -- POP")
        (let ((top (car s)))
          (set! s (cdr s))
          top)))
    (define (initialize)
    ...
    (define (dispatch message)
      (cond ((eq? message 'push) push)
            ((eq? message 'pop) (pop))
            ((eq? messasge 'initialize) (initialize))
...
(define (lookup-label labels label-name)
  (let ((val (assoc label-name labels)))
    (if val
      (cdr val)
      (error "Undefined label -- ASSEMBLER lookup-label" label-name))))

;; Dispatch

(define (make-execution-procedure inst labels machine pc flag stack ops)
  (cond ((eq? (car inst) 'assign)
         (make-assign inst machine labels op pc))
        ((eq? (car inst) 'test)
         (make-test inst machine labels op flag pc))
        ((eq? (car inst) 'branch)
        ...
        (else (error "Unknown instruction type -- make-execution-procedure" inst))))
...
;; GOTO TEST BRANCH
(define (make-test inst machine laels operations flag pc)
...
(define (make-goto inst machine labels pc)
  (let ((dest (goto-dest inst)))
    (cond ((label-exp? dest)
          ...
          (else (error "bad GOTO instrucion -- ASSEMBLE" inst)))))
...
;; peform operation
(define (make-peform inst machine labels operations pc)
  (let ((action (peform-action inst)))
    (if (operation-exp? action)
      (let ((action-proc (make-operation-exp action machine labels operations)))
        (lambda ()
          (action-proc)
          (advance-pc pc)))
      (error "Bad Peform instruction -- ASSEMBLE" inst))))
(define (peform-action inst)
  (cdr inst))
